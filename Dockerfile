...
RUN if [ "${SKIP_DOWNLOAD}" = "true" ]; then \
        ...
        && wget -q -c "${MINECRAFT_MOD_PACKAGE}" -O "/tmp/feed-the-beast/${MINECRAFT_MOD_NAME}_Server.zip" \
        && unzip "/tmp/feed-the-beast/${MINECRAFT_MOD_NAME}_Server.zip" -d /tmp/feed-the-beast \
        && chmod -R 777 /tmp/feed-the-beast \
        && chown -R minecraft /tmp/feed-the-beast \
        && bash -x /tmp/feed-the-beast/Install.sh \
        && chmod -R 777 /tmp/feed-the-beast \
        && chown -R minecraft /tmp/feed-the-beast; \
    fi
